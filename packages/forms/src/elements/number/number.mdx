...
import { FFInputNumber, FFInputNumberWithCheckbox } from './number';
...
### Number field with checkbox

```js
import { FFInputNumberWithCheckbox } from '@tpr/forms';
<form noValidate>
...
required=true;
```
				...
				{({ handleSubmit }) => (
					<form onSubmit={handleSubmit} noValidate>
						<FFInputNumberWithCheckbox
							...
							checkboxProps={{ name: 'checkbox', label: 'Not available' }}
						/>
					</form>
				)}
			</Form>
